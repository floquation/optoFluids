...
#
##
# This script extracts the particlePositions from openFOAM results within in small volume defined by a cylinder with a ellipsoidal base. Used to obtain the particles from the interrogation zones
##
...
b = 0
#
usageString = "   usage: " + sys.argv[0] + " -i <foamCase dir> -C <center of interrogationZone cylinder along X-dir: '(float float)'> -a <y-dir halfwidth of interrogation zone ellipsoid-cylinder>  -b <z-dir halfwidth of interrogation zone ellipsoid-cylinder>\n" \
...
try:
    opts, args = getopt.getopt(sys.argv[1:],"hfi:o:c:C:a:b:")
...
if a == 0 or b == 0:
    print usageString 
    print "    Note: interrogation zones unspecified: a and b cannot be 0"
...
def convertSyntax( posFileNameIn, posFileNameOut ):
    ...
    for line in posFileIn :
		...
		if ((((yCoord - C[0])/a)**2 + ((zCoord - C[1])/b)**2) < 1): # then the current line has particle in the given interrogation zone
			particleCount += 1
...
for itemNum in lijstje :
	...
	#print itemNum - prevTime
	if( itemNum - prevTime > 100*2.5e-6): #then the timetag should iterate. For this to work, lijstje needs to be sorted by number size!
		timeTag += 1
	#diagnostic output
        #print timeTag
...
#
#
# EOF particleExtractorEllipsoidalCylinder.py
