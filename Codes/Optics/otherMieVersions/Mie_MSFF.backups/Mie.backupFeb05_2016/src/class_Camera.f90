...
!	Converts r1, r2, nPixel1 and nPixel2 into rp(3,nPixel1*nPixel2): the positions of all pixels.
    subroutine generatePixels(this)
        ...
        call debugmsg(5, "Camera","r = ", this%rp)
    end subroutine
...
! \************************/
    ...
    subroutine addEField(this, eField)
        ...
        call debugmsg(8,"Camera","eField_in   size1 = ", size(eField,1))
        call debugmsg(8,"Camera","eField_in   size2 = ", size(eField,2))
        call debugmsg(8,"Camera","eField_this size1 = ", size(this%eField,1))
        call debugmsg(8,"Camera","eField_this size2 = ", size(this%eField,2))
        ...
        call debugmsg(7,"Camera","eField_before = ", this%eField)
        this%eField = this%eField + eField
        call debugmsg(6,"Camera","eField_after = ", this%eField)
...
!  If we use this equation to compute the intensity, this factor will drop out anyway -> I /= I(t)
    function getIntensity(this) result(intensity)
        ...
        do pixel = 1, nPixels
            call debugmsg(8,"Camera","eField(xyz,pixel) = ", this%eField(:,pixel))
            intensity(pixel) = dot_product(this%eField(:,pixel),this%eField(:,pixel)) ! I propto |E|^2
            call debugmsg(8,"Camera","intensity(pixel) = ", intensity(pixel))
        end do
        call debugmsg(5,"Camera","intensity = ", intensity)
