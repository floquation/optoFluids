...
module DEBUG
    ...
    interface debugmsg
        module procedure debugmsg_, debugmsg_real4, debugmsg_real8, debugmsg_int, debugmsg_logical, &
            debugmsg_cmplx, debugmsg_int4vec, debugmsg_real4vec, debugmsg_real8vec, debugmsg_cmplxvec, &
...
! Scalars
    ...
    subroutine debugmsg_logical(lvl, caller, msg, val)
        character (len=*), intent(in) :: caller, msg
        integer, intent(in) :: lvl
        logical, intent(in) :: val
        if(lvl <= debugLevel) write(6,*) trim(getDebugMsgHeader(lvl,caller)) // " " // trim(msg), val
    end subroutine debugmsg_logical
    
! Vectors

    subroutine debugmsg_int4vec(lvl, caller, msg, val)
        character (len=*), intent(in) :: caller, msg
        integer, intent(in) :: lvl
        integer*4, intent(in) :: val(:)
        if(lvl <= debugLevel) write(6,*) trim(getDebugMsgHeader(lvl,caller)) // " " // trim(msg), val
    end subroutine debugmsg_int4vec
