...
subroutine scatter(X,REFREL,S_ANGLES,S1,S2)
...
!    S_ANGLES = Array of the scattering angles to-be-computed
!                             
! Returns:                                                              
!    S1(size(S_ANGLES)) = -i*f_22 (incid. E perp. to scatt. plane,        
!                                scatt. E perp. to scatt. plane)        
!    S2(size(S_ANGLES)) = -i*f_11 (incid. E parr. to scatt. plane,        
...
! Local variables:
  ...
  double precision :: CHI,CHI0,CHI1,DX,EN,FN,P,PSI,PSI0,PSI1, &
       THETA,XSTOP,YMOD                                 
  double precision :: AMU(SIZE(S_ANGLES)),PI(SIZE(S_ANGLES)),PI0(SIZE(S_ANGLES)),PI1(SIZE(S_ANGLES)),  &
       TAU(SIZE(S_ANGLES))                                      
...
! Safety checks                                                      
  if(SIZE(S_ANGLES) /= SIZE(S1) .OR. SIZE(S_ANGLES) /= SIZE(S2))		&
	stop 'Error: S_ANGLES, S1 and S2 must be of the same size'
