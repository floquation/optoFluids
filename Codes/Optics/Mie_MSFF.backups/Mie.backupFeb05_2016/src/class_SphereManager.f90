...
!        procedure, public :: computeCosScatteringAngles
...
! Initialises all spheres based on the given input positions
    subroutine initSpheres(this, spherePos)
        ...
        do i = 1, numSpheres
            ...
            call debugmsg(7,"SphereManager","Post-creation: Created the sphere " // this%sphereList(i)%sphPtr%toString())
        end do
...
! #UNUSED       
    subroutine computeCosScatteringAngles(this, psph, r_trgt, khat, CosSAngles)
        class(SphereManager) :: this
	...
	real*8, allocatable, intent(out) :: CosSAngles(:)      ! All scattering angles which require computation (numAngles)
        
        integer :: i, numAngles
        real*4 :: rsph(3), dr(3)
        
        numAngles = size(r_trgt,2)
        allocate(CosSAngles(numAngles))
...
!                call debugmsg(3, "SphereManager","rsph = ",rsph)
    ...
    ! Determine the scattering angle for each target
                ...
                CosSAngles(i) = (dot_product(dr,khat)/&
                    sqrt(dot_product(dr,dr)))
                call debugmsg(3, "SphereManager","SAngles(i) = acos(<dr,khat>/<dr,dr>) = ", acos(CosSAngles(i))*180/3.141592654)

        end do
        
       ! Return scattering angles (SAngles) and scattering planes (Eihat_p, Eihat_l and Eshat_l) 
    end subroutine computeCosScatteringAngles
