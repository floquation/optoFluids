...
contains
    ...
    subroutine readParameters(inputfile, refre, refim, refmed, wavel, rad, khat, Eihat, spherePos, &
                                nPixel1, nPixel2, r0, r1, r2, conv_minp, conv_maxp, dop0, strategyKeyword)
	...
	! Scattering strategy parameter
		...
		character(len = 250) :: strategyKeywordBuffer
...
!        print *, prtcPosFile
    ...
    ! Output
        ...
        call skipcomments (10, line,'!')
        read(line, fmt='(A)') pixelCoordsOutFile 
        call removecomment(pixelCoordsOutFile,'!','\')
        call debugmsg(3, "IO","pixelCoordsOutFile = " // trim(pixelCoordsOutFile))
        call skipcomments (10, line,'!')
        read(line, fmt='(A)') intensityOutFile
        call removecomment(intensityOutFile,'!','\')
        call debugmsg(3, "IO","intensityOutFile = " // trim(intensityOutFile))
		call skipcomments(10, line,'!')
		read(line, fmt='(A)') strategyKeywordBuffer
		call removecomment(strategyKeywordBuffer,'!','\')
		allocate(strategyKeyword, source = trim(strategyKeywordBuffer))
		call debugmsg(3, "IO","strategyKeyword = " // strategyKeyword)
