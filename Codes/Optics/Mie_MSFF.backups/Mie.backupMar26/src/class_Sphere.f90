module class_Sphere
    ...
    type, public :: Sphere
        ...
        complex, allocatable, public :: S1(:) ! size = (n(n-1)/2-(n-1)) = number of terms in lwr triangle - terms with j=i .or. j'=i
        complex, allocatable, public :: S2(:) ! size = (n(n-1)/2-(n-1)) ""
        complex, public :: S1_bs(1) ! size = (1)
        complex, public :: S2_bs(1) ! size = (1)
        ...
        ! Scientific computing code... A lot faster if these are public.
    contains
        private
        final :: destroy
...
! \************************/
    ...
    subroutine allocateS(this,Nsph)
        ...
        integer, intent(in) :: Nsph ! Number of spheres incl. self
          call debugmsg(4,"Sphere","Allocating [S] with size: ", Nsph*(Nsph-1)/2 - (Nsph-1))
        allocate(this%S1(Nsph*(Nsph-1)/2 - (Nsph-1)),this%S2(Nsph*(Nsph-1)/2 - (Nsph-1)))
    ...
    subroutine deallocateS(this)
        class(Sphere) :: this
          call debugmsg(5,"Sphere","Deallocating S for " // this%toString())
