...
#
# SCRATCH ALL THE BEFORE STUFF: UNIFORMLY  DISTRIBUTED PARTICLES IN ALL DIRECTIONS
...
usageString = "   usage: " + sys.argv[0] + " -i <accumProbFunctionFileName for r> -o <output fileName>" \
            + " [-N <numParticles>] [-W <width of beam>] [-L <length of beam>] [-H <height of beam]\n" \
            ...
            + "       -H (float) := height of the beam in which the particles are generated.. can be negative, should rather not. Defaults to 1.\n"\
	    + "	-W (float) := width of the beam in which the particles are generated.. can be negative, should rather not. Defaults to 1.\n"\
...
try:
    opts, args = getopt.getopt(sys.argv[1:],"hfi:o:N:W:L:H:O:")
