#! /usr/bin/python
#
# meanWeightedOfFiles.py
#
# Takes a directory of data files, which must have the same 1D vector or 2D array format, and takes an element-wise weighted mean of those files.
#  This is like "meanOfFiles.py", but now normalises the data to a mean of 1 before adding them up.
#  Then the new mean is rescaled to the global mean among all files.
#
# Kevin van As
#  June 30th 2015
#
...
for intensityFN in os.listdir(intensityDN) :
    ...
    if firstFile:
        ...
        thisMean=np.mean(dataIn)
        globalMean=thisMean # Accumulate the mean of all files
        dataAccum=dataIn/thisMean # mean=1 data
        numTerms=1
    else:
        if np.shape(dataIn) == fileShape:
            thisMean=np.mean(dataIn)
            globalMean=globalMean+thisMean # Accumulate the mean of all files
            dataAccum=dataAccum+dataIn/thisMean # add mean=1 data
            numTerms=numTerms+1
        else:
...
globalMean=globalMean/numTerms # Now it is the global mean, instead of the accumulation
dataAv=dataAccum/numTerms*globalMean
#print dataAv
